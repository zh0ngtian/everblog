...
module.exports = class EverblogCore {
  ...
  * findNotes(_notebookGuid, _filter, _offset, _maxNotes, _notes) {
    ...
    notes = yield notes.map(function* (note) {
      ...
      let targetFileName = noteName + ' | ' + notebookName
      targetFolderNames.push(targetFileName)
      ...
      note.category = notebookName
      note.top = false
      if (note.tagGuids) {
        let all_tags = yield this._getNoteTagNames(note.guid)
        let real_tags = []
        for (let tag of all_tags) {
          if (tag !== 'published' && tag !== 'top') {
            real_tags.push(tag)
          }
        }
        note.tags = real_tags
        if (all_tags.includes('published') && all_tags.includes('top')) {
          note.top = true
        }
      } else {
        note.tags = []
    ...
    if (fs.existsSync('source/images/')) {
      ...
      for (let existingFolderName of existingFolderNames) {
        if (existingFolderName[0] !== '.' && !targetFolderNames.includes(existingFolderName)) {
